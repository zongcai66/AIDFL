...
logging.basicConfig(filename='CIFAR10_Krum_3_data.txt',
...
def entropy_loss(weights):
    entropy = torch.sum(
        -F.softmax(weights, dim=0) * F.log_softmax(weights, dim=0))
    return entropy
...
def get_weight_x_data(client, batched_data):
    # raw_data=batched_data.view(-1, 1024)
    raw_data = client.model(batched_data)
...
def detect_outliers(data_tensor, lower_threshold, upper_threshold):
    ...
    outliers = torch.where(
        (data_tensor < lower_threshold) | (data_tensor > upper_threshold))[0]
...
#
#     average_weights_sum = torch.sum(
#         torch.stack(client_weights_sum_queue.current_items())) / len(
#         client_weights_sum_queue)
#
#     param_ = []
...
def krum_aggregate(models):
    ...
    accum = (model_list[0][0]).copy()
    for layer in accum:
        accum[layer] = torch.zeros_like(accum[layer])

    distance_list = [0] * num_models
    min_index = 0
    ...
    selected_model, _ = model_list[min_index]
    for layer in selected_model:
        accum[layer] = selected_model[layer]
...
def adjust_weight(ID):
    client_weights_sum_queue = eval(f"client_weights_sum_queue_0{ID}")
    client_agg_weights_queue = eval(f"client_weights_sum_queue_0{ID}")

    models = {}
    for i, weights in enumerate(client_agg_weights_queue.current_items()):
        model_dict = {f'layer_{i}': w for i, w in enumerate(weights)}
        models[i] = (model_dict, 1)

    aggregated_weights = krum_aggregate(models)
...
def client_train_and_sync(client, data_loader_4_client, num_epochs):
    for _ in range(num_epochs):
        ...
        for images, labels in data_loader_4_client:
            ...
            # logging.info(f'client id: {client.ID}',f'Loss: {loss_all.item():.4f}')
            ...
            mi_lower_threshold, mi_upper_threshold = get_threshold_mi_or_ce(
                diff_mi)
            ce_lower_threshold, ce_upper_threshold = get_threshold_mi_or_ce(
                diff_ce)
            mi_anomaly = detect_outliers(diff_mi, mi_lower_threshold,
                                         mi_upper_threshold)
            ce_anomaly = detect_outliers(diff_ce, ce_lower_threshold,
                                         ce_upper_threshold)
            normal_samples_id = get_normal_samples(len(diff_ce), mi_anomaly,
                                                   ce_anomaly)
            sample_weight = get_sample_weight(mi_records, ce_records)
            sample_weight_x_model_weight = get_sample_weight_x_model_weight(
                sample_weight, normal_samples_id, client)
            append_client_agg_weights(client.ID, sample_weight_x_model_weight)
            acclumated_sample_weight = get_acclumated_sample_weight(
                sample_weight, normal_samples_id)
...
client_subset_01= ChangeableSubset(
    cifar10_full,clients_index[0], label_flipping=False, data_poisoning=True, poisoned_persent=1,
    poisoned_ratio=1, targeted=False, noise_type='salt')
client_subset_02= ChangeableSubset(
    cifar10_full,clients_index[2], label_flipping=False, data_poisoning=True, poisoned_persent=1,
    poisoned_ratio=1, targeted=False, noise_type='salt')
client_subset_04= ChangeableSubset(
    cifar10_full,clients_index[4], label_flipping=False, data_poisoning=True, poisoned_persent=1,
    poisoned_ratio=1, targeted=False, noise_type='salt')
