...
logging.basicConfig(filename='mnist_AIDFL_7_label_iid.txt', level=logging.INFO, format='%(asctime)s - %(message)s')
...
# split MNIST to non-IID datasets
def split_mnist_non_iid(dataset, num_clients):
    # group MNIST by label
...
def get_weight_x_data(client,batched_data):
    ...
    # normalization
    raw_normalized = (raw_data - mean) / (std + 1e-10)
...
test_dataset = torchvision.datasets.MNIST(root='./data', train=False, download=False, transform=ToTensor())
test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=64, shuffle=False)


# 加载 MNIST 数据集
mnist_full = MNIST(root='./data', train=True, download=False, transform=ToTensor())
#
# index_1 = range(60000)
...
data_loaders,clients_index = split_mnist_non_iid(mnist_full,num_clients)

client_subset_01= ChangeableSubset(
    mnist_full,clients_index[0], label_flipping=True, data_poisoning=False, poisoned_persent=1, targeted=False)
client_subset_02= ChangeableSubset(
    mnist_full,clients_index[2], label_flipping=True, data_poisoning=False, poisoned_persent=1, targeted=False)
client_subset_03= ChangeableSubset(
    mnist_full,clients_index[3], label_flipping=True, data_poisoning=False, poisoned_persent=1, targeted=False)
client_subset_04= ChangeableSubset(
    mnist_full,clients_index[4], label_flipping=True, data_poisoning=False, poisoned_persent=1, targeted=False)
client_subset_05= ChangeableSubset(
    mnist_full,clients_index[6], label_flipping=True, data_poisoning=False, poisoned_persent=1, targeted=False)
client_subset_06= ChangeableSubset(
    mnist_full,clients_index[8], label_flipping=True, data_poisoning=False, poisoned_persent=1, targeted=False)
client_subset_07= ChangeableSubset(
    mnist_full,clients_index[9], label_flipping=True, data_poisoning=False, poisoned_persent=1, targeted=False)
